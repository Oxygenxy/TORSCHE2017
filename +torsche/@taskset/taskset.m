classdef taskset < torsche.schedobj&handle
    ...
    methods
        function setoftasks = taskset(varargin)
            ...
            if nargin==3
                if isa(varargin{1},'torsche.taskset') && ischar(varargin{2}) 
                    setoftasks = set_helper(varargin{:});
                    return;
                end
            end
            ...
            if ni >= 1
                ...
                if isa(varargin{1},'torsche.taskset') 
                    tasks = varargin{1}.tasks;
                ...
                elseif isa(varargin{1},'torsche.job') 
                     tasks = varargin{1}.parent.tasks;
                    % Precedencs constrains
                    if (ni > 1)
                        prec = varargin{length(varargin)};
                    else
                        prec = varargin{1}.parent.Prec;    
                    end
                elseif iscell(varargin{1}) 
                    if size(varargin{1},1) == 1
                        for i = 1:length(varargin{1})
                            if ~isa(varargin{1}{i},'torsche.task')
                ...
                elseif isnumeric(varargin{1}) 
                    ...
                    if size(ProcTime,1) == 1
                        ...
                        for i = 1:length(ProcTime)
                            nts=[nts ' torsche.task(' int2str(ProcTime(i)) ')'];
                        end
                        nts=[nts ' };'];
                        eval(nts);
                        % Precedencs constrains
                ...
                % input is graph    
                elseif(isa(varargin{1},'torsche.graph'))
            ...
            prec = full(prec);
            prec = prec(1:min(length(tasks),size(prec,1)), 1:min(length(tasks),size(prec,2)));
            [i,j,s] = find(prec);
            m = max(length(prec), length(tasks));
            ...
            setoftasks.parent = torsche.schedobj;
            setoftasks.tasks = tasks;
            setoftasks.Prec = prec;
            setoftasks.schedule = schedule;
            setoftasks.Version = 0.3;
            setoftasks.TSUserParam = TSUserParam;
